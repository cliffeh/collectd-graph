...
function RRDGraph(parent, id, data, settings) {
    /**
     * data: 
     *
     * { GRAPHNAME:           
    ...
    this.leftMarginWidth = settings['left-margin'] ? settings['left-margin'] : 50;
    this.rightMarginWidth = settings['right-margin'] ? settings['right-margin'] : 0;

    // note: total height = height + tmargin + bmargin
    this.height = settings['height'] ? settings['height'] : 300;
    this.bottomMarginHeight = settings['bottom-margin'] ? settings['bottom-margin'] : 50;
    ...
    $('#dash').svg(
	{
	    settings : {
		'id' : this.id,
		'width' : this.width + this.leftMarginWidth + this.rightMarginWidth,
		'height' : this.height + this.topMarginHeight + this.bottomMarginHeight
	    }
	});
    this.svg = $('#dash').svg('get');

    // containers for the chart, lines, and margins
    this.chart = this.svg.group(this.id + 'chart', {
	'transform' : 'translate(' + this.leftMarginWidth + ' ' + this.topMarginHeight + ')'
    });
    this.chartLines = new Array();

    this.lmargin = this.leftMarginWidth ? 
	this.svg.group(this.id + 'lmargin', {
	    'transform' : 'translate(0 ' + this.topMarginHeight + ')'
	}) : null;
    this.rmargin = this.rightMarginWidth ? 
	this.svg.group(this.id + 'rmargin', {
	    'transform' : 'translate('
		+ (this.leftMarginWidth + this.width) + ' '
		+ this.topMarginHeight + ')'
	}) : null;
    this.tmargin = this.topMarginHeight ? 
	this.svg.group(this.id + 'tmargin', {
	    'transform' : 'translate(' + this.leftMarginWidth + ' 0)'
	}) : null;
    this.bmargin = this.bottomMarginHeight ? 
	this.svg.group(
	    this.id + 'bmargin', {
...
RRDGraph.prototype.draw = function() {
    ...
    var xScale = this.width / (this.data[this.data.length - 1][0] - this.data[0][0]);
